...
// ========================== ACavity2DFemWorker ========================
struct ACavity2DCbsWorker{
	ACavity2DCbsWorker(const IGrid& grid, double Re, double tau_coef);
...
ACavity2DCbsWorker::ACavity2DCbsWorker(const IGrid& grid, double Re, double tau_coef):
...
void ACavity2DCbsWorker::initialize_saver(std::string stem){
	_writer.reset(new VtkUtils::TimeSeriesWriter(stem));
};

void ACavity2DCbsWorker::save_current_fields(size_t iter){
	if (_writer){
...
FemAssembler ACavity2DCbsWorker::build_fem(unsigned power, const IGrid& grid){
...
void ACavity2DCbsWorker::to_next_iteration(
		const std::vector<double>& tau,
		const std::vector<double>& beta,
		std::vector<double>& u_new,
		std::vector<double>& v_new,
...
void ACavity2DCbsWorker::convergence_report(size_t iter){
...
bool ACavity2DCbsWorker::is_converged(double eps) const{
	return eps > _n2_dudt 
		&& eps > _n2_dvdt
		&& eps > _n2_dpdt;
}

void ACavity2DCbsWorker::apply_velocity_bc(double u_top, std::vector<double>& u, std::vector<double>& v) const {
...
void ACavity2DCbsWorker::compute_delta_u_1(
		const std::vector<double>& tau,
	...
	for (size_t ielem=0; ielem < _fem.n_elements(); ++ielem){
		...
		std::vector<Vector> vel = _fem.local_vector(ielem, _u, _v);
		// convection with cg stabilization
		{
			std::vector<double> local = elem.integrals->custom_matrix([&](size_t i, size_t j, const IElementIntegrals::OperandArg* arg){
				...
				double div = arg->divergence(vel);
				double s = tau[ielem] / 2.0;
				double div_u_phi_i = dot_product(v1, arg->grad_phi(i)) + arg->phi(i) * div;
				double div_u_phi_j = dot_product(v1, arg->grad_phi(j)) + arg->phi(j) * div;
				return div_u_phi_j * (arg->phi(i) + s * div_u_phi_i) * arg->modj();
			});
...
void ACavity2DCbsWorker::compute_delta_u_2(
		const std::vector<double>& tau,
		const std::vector<double>& p_new,
		std::vector<double>& delta_u, std::vector<double>& delta_v) const {
	...
	for (size_t ielem=0; ielem<_fem.n_elements(); ++ielem){
		...
		// -dp/dx  with cg stabilization
		{
			std::vector<double> local = elem.integrals->custom_matrix([&](size_t i, size_t j, const IElementIntegrals::OperandArg* arg){
				...
				double s = tau[ielem] / 2.0;
				double div_u_phi_i = dot_product(v1, arg->grad_phi(i)) + arg->phi(i) * div;
				return arg->grad_phi(j).x() * (arg->phi(i) + s * div_u_phi_i) * arg->modj();
			});
			_fem.add_to_global_vector(-1.0, ielem, local, p_new, rhs_u);
		}
		// -dp/dy  with cg stabilization
		{
			std::vector<double> local = elem.integrals->custom_matrix([&](size_t i, size_t j, const IElementIntegrals::OperandArg* arg){
				Vector v1 = arg->interpolate(vel);
				double div = arg->divergence(vel);
				double s = tau[ielem] / 2.0;
				double div_u_phi_i = dot_product(v1, arg->grad_phi(i)) + arg->phi(i) * div;
				return arg->grad_phi(j).y() * (arg->phi(i) + s * div_u_phi_i) * arg->modj();
			});
...
class SemiImplicitCbsWorker: public ACavity2DCbsWorker{
public:
	SemiImplicitCbsWorker(const IGrid& grid, double Re, double tau_coef=0.7):
			ACavity2DCbsWorker(grid, Re, tau_coef){
...
TEST_CASE("Cavity 2D, FEM CBS algorithm", "[cavity2-fem-si-cbs]"){
	std::cout << std::endl << "--- cfd_test [cavity2-fem-si-cbs] --- " << std::endl;
	...
	worker.initialize_saver("cavity2-cbs");
...
class ExplicitCbsWorker: public ACavity2DCbsWorker{
public:
...
TEST_CASE("Cavity 2D, FEM Explicit CBS algorithm", "[cavity2-fem-ex-cbs]"){
	std::cout << std::endl << "--- cfd_test [cavity2-fem-ex-cbs] --- " << std::endl;
